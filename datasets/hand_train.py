"""Training pairs for Hand MLMF global branch (offline-augmented template)."""
...
def GetHandDatasetfromfile(train):
    ...
    ImageSet = sorted(os.listdir(PathOfData))
    MaskSet = sorted(os.listdir(PathOfTarget))

    ImageSetfinal, MaskSetfinal = [], []
    ...
    return list(zip(ImageSetfinal, MaskSetfinal))


class HandTrainDataset(data.Dataset):
    """37 landmarks, coords [y, x] in augmented template space."""

    def __init__(self, istrain, original_size, load_size):
        self.data = GetHandDatasetfromfile(train=istrain)
        ...
        self.original_size = original_size  # [H, W] of augmented crops (template)
    ...
    def __getitem__(self, index):
        ...
        if self.load_size is not None:
            pil_image = transforms.Resize(
                self.load_size, interpolation=transforms.InterpolationMode.LANCZOS)(img_x)

        image = self.prep(pil_image)
        image_y, image_x = image.shape[-2], image.shape[-1]
