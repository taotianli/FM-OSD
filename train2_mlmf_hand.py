# Fine-tune MLMF local branch — Hand dataset (37 landmarks, variable resolution)
import argparse
import torch
...
def find_landmark_all_mlmf_local_hand(
    ...
    with torch.no_grad():
        ...
        for i in range(len(lab)):
            ...
            d1_post_loc = model_post(
                d1_loc_list, np1l, ls1l, islocal=True)
        ...
        for sample in tqdm(dataloader, desc="Eval-hand"):
            ...
            desc2_list = extractor.extract_multi_layer_multi_facet_descriptors(
                ...
                layers=mlmf_config['layers'], facets=mlmf_config['facets',] if False else mlmf_config['facets'], bin=bin)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--facet', default='key')
    ...
    parser.add_argument('--exp', default='local_mlmf_hand')
    ...
    parser.add_argument('--global_ckpt', type=str, default='',
                        help='Path to train1_mlmf_hand best checkpoint (fusion / conv_out keys)')
    ...
    one_shot = Hand_SSL_Infer(
        pathDataset=args.dataset_pth, mode='Oneshot', id_oneshot=args.id_shot)
    _, landmarks_temp, img_path_temp, _ = one_shot.__getitem__(0)
    ...
    dataloader_test = DataLoader(
        dataset_test, batch_size=1, shuffle=False, num_workers=2, collate_fn=_coll)
    ...
    model_post.eval()
    pred_all, gt_all = find_landmark_all_mlmf_local_hand(
    ...
    ev = Evaluater(pred_all, gt_all, args.eval_radius, eval_dir,
                   name='mlmf_hand_local_init', spacing=[0.1, 0.1])
    ev.calculate()
    ev.cal_metrics()
    best_mre = ev.mre
        ...
        for images, labs, lab_smalls, image_paths in train_loader:
            ...
            for j in range(labs.shape[1]):
                ...
                desc_post_local = model_post(
                    desc_local_list, np_l, ls_l, islocal=True)
            ...
            if iter_num % 20 == 0:
                ...
                pred_all, gt_all = find_landmark_all_mlmf_local_hand(
                    ...
                    template_orig_size=template_orig_size, topk=args.topk)
                ev = Evaluater(pred_all, gt_all, args.eval_radius, eval_dir,
                               name=f'mlmf_hand_local_{iter_num}', spacing=[0.1, 0.1])
                ev.calculate()
                ev.cal_metrics()
                mre = ev.mre
                if mre < best_mre:
